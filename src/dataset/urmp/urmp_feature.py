...
#Added https://stackoverflow.com/questions/68679806/attributeerror-cant-pickle-local-object-pre-datasets-locals-lambda-when
import dill

sys.path.insert(1, os.path.join(sys.path[0], '../../..'))

from src.utils.utilities import (mkdir, float32_to_int16, freq2note, get_filename, get_process_groups, read_lst, write_lst)
...
def remove_empty_segment(wav, frame_roll, frames_per_second, sample_rate, notes_num):
	segments = []
	samples_per_frame = sample_rate * 1. / frames_per_second
	for i in range(frame_roll.shape[-1]):
		if not frame_roll[i] == notes_num:
...
def pack_urmp_dataset_to_hdf5(args):
	...
	print("I have audio_groups")
	for pid, (st, ed) in enumerate(audio_groups):
		process_group(st, ed, audios_num, pid)
		
		#Do not use multiprocessor
...
if __name__ == '__main__':
	...
	parser.add_argument('--n_fft', type=int, required=True, help='Number of fft bins.')
	parser.add_argument('--frames_per_second', type=int, required=True, help='Number of frames per second.')
	parser.add_argument('--begin_note', type=int, required=True, help='Begin note.')
	parser.add_argument('--notes_num', type=int, required=True, help='Numbre of notes.')
